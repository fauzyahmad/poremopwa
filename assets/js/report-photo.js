var pictureDetailData = document.querySelector('.picture-progress');
var bearerToken = localStorage.getItem('access_token');

function clearDatas() {
	while (pictureDetailData.hasChildNodes()) {
        pictureDetailData.removeChild(pictureDetailData.lastChild);
    }
}

function createListPicture(data) {
	for (var i in data.reportItem.report_photos) {
		pictureDetailData.innerHTML += 
		`<li class="picture-boq mdlext-lightboard__slide" data-details="[
                      { 'name': 'Progress', 'value': '${data.reportItem.percentage}%'}]">
                      <a href="api_poremo/public${data.reportItem.report_photos[i].path}" class="mdlext-lightboard__slide__frame">
                        <figure>
                          <img src="api_poremo/public${data.reportItem.report_photos[i].path}" title="${data.reportItem.updated_at}">
                          <figcaption>${data.reportItem.updated_at}</figcaption>
                        </figure>
                      </a>
                    </li>`;
	}
...
function createListCachePicture(argument) {
	for (var i in data.report_photos) {
		pictureDetailData.innerHTML +=
		`<li class="picture-boq mdlext-lightboard__slide" data-details="[
                      { 'name': 'Progress', 'value': '${data.percentage}'}]">
                      <a href="${data.report_photos[i].path}" class="mdlext-lightboard__slide__frame">
                        <figure>
                          <img src="${data.report_photos[i].path}" title="Gambar">
                          <figcaption>Tanggal dan Waktu</figcaption>
                        </figure>
                      </a>
                    </li>`;
	}
...
var idURL = localStorage.getItem('photo_report-item'); 
var baseUrl = 'api_poremo/public/api/v1/report/item';
var networkDataReceived = false;

fetch(baseUrl+'/'+idURL+'/photos', {
  headers: {
  ...
  clearAllData('data-report_photo');
	// clearDatas();
	createListPicture(data);
	return dbPromise
        .then(function (db) {
        var tx = db.transaction('data-report_photo', 'readwrite');
        var store = tx.objectStore('data-report_photo');
        return Promise.all(data.reportItem.report_photos.map(function(itemPhoto) {
            console.log('Adding item: ', itemPhoto);
            store.put(itemPhoto);
            return tx.complete;
           })
         )

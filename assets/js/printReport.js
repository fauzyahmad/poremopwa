...
var reportHeading = document.querySelector('#heading');
var url = 'api_poremo/public/api/v1/report/project/';
...
var reportDataHeadingHTML = '';
var reportDataBodyHTML = '';
...
function createListReportHeading(data) {
	
	for(var i in data.reports) {
		reportDataHeadingHTML = 
		`<tr>
		  <th style="color:white" class="mdl-data-table__cell--non-numeric">Tanggal</th>
		`;
		for(var j in data.reports[i].report_items) {
			reportDataHeadingHTML += 
			`
			  <th style="color:white">${data.reports[i].report_items[j].boq_item_id}</th>
			`;
		}
	}
	// reportDataHeadingHTML = '</tr>';
	reportHeading.innerHTML = reportDataHeadingHTML;
}

function createListReportBody(data) {
	for(var i in data.reports) {
		reportDataBodyHTML += 
		`<tr>
		...
		for(var j in data.reports[i].report_items) {
			reportDataBodyHTML +=
			`<td>${data.reports[i].report_items[j].percentage}%</td>`
		}
		reportDataBodyHTML += `</tr>`;
	}
	reportData.innerHTML = reportDataBodyHTML;
...
fetch(url+projectId, {
	...
	createListReportHeading(data);
	createListReportBody(data);
})
